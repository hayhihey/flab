...
export const getPlacePredictions = async (input, bias, maxResults = 5, sessionToken) => {
    // Try Legacy AutocompleteService if available
    const hasLegacy = window.google?.maps?.places?.AutocompleteService;
    if (hasLegacy) {
        return new Promise((resolve) => {
            ...
            if (bias && window.google?.maps) {
                ...
                options.location = new window.google.maps.LatLng(bias.lat, bias.lng);
